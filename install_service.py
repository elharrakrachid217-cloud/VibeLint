...
install_service.py
==================
Register VibeGuard as a background service on Windows, macOS, or Linux.
...
What it does:
  ...
  - Registers VibeGuard's server.py to start automatically at login
  - Redirects output to vibeguard/logs/vibeguard.log
...
SERVER_PY = VIBEGUARD_DIR / "server.py"
PYTHON_EXE = Path(sys.executable).resolve()
LOG_DIR = VIBEGUARD_DIR / "logs"
LOG_FILE = LOG_DIR / "vibeguard.log"
...
def _install_windows() -> bool:
    ...
    try:
        ...
        if r.returncode == 0:
            _step("✓ Task started — VibeGuard is running")
        else:
...
def _install_mac() -> bool:
    ...
    try:
        ...
        if r.returncode == 0:
            _step("✓ Service loaded — VibeGuard is running")
        else:
...
def _install_linux() -> bool:
    ...
    unit = (
        "[Unit]\n"
        "Description=VibeGuard MCP Security Server\n"
        "After=network.target\n\n"
        "[Service]\n"
        "Type=simple\n"
        f"ExecStart={PYTHON_EXE} {SERVER_PY}\n"
        f"WorkingDirectory={VIBEGUARD_DIR}\n"
    ...
    try:
        ...
        _step("✓ Service started — VibeGuard is running")
    except FileNotFoundError:
...
def main() -> None:
    print("🛡️  VibeGuard Service Installer")
